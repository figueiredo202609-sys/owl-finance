...
export default function ExpensesPage() {
  ...
  const loadItems = useCallback(() => {
    setLoading(true);
    api.get('/client/transactions?type=expense')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modal === 'create') {
        await api.post('/client/transactions', { type: 'expense', value: parseFloat(form.value), date: form.date, name: form.name, description: form.description });
                    ...
                    <tr key={item.id} className="hover:bg-white/3 transition-colors" data-testid={`expense-row-${item.id}`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button data-testid={`delete-expense-${item.id}`} onClick={() => setConfirmDelete(item)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-400 hover:bg-red-400/10 transition-colors"><Trash2 size={15} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
