...
import { Plus, Pencil, Trash2, TrendingDown, X } from 'lucide-react';
...
export default function ExpensesPage() {
  ...
  const fetch = useCallback(() => {
    setLoading(true);
    api.get('/client/transactions?type=expense').then(r => setItems(r.data)).catch(() => toast.error('Erro ao carregar')).finally(() => setLoading(false));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const payload = { type: 'expense', value: parseFloat(form.value), date: form.date, name: form.name, description: form.description };
      if (modal === 'create') {
    ...
    <div className="flex h-screen bg-[#020617]">
      ...
      <main className="flex-1 overflow-y-auto p-8 page-enter">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-white text-3xl font-black tracking-tight" style={{ fontFamily: 'Manrope' }}>Despesas</h1>
              <p className="text-slate-400 mt-1">Todas as saídas registradas</p>
            </div>
            <button data-testid="add-expense-btn" onClick={openCreate} className="btn-pill px-5 py-2.5 flex items-center gap-2 text-sm">
              ...
              <table className="w-full text-sm" data-testid="expenses-table">
                ...
                <tbody className="divide-y divide-white/5">
                  {loading ? (
                    ...
                    <tr><td colSpan={5} className="text-center py-12 text-slate-500">Nenhuma despesa registrada ainda</td></tr>
                  ) : items.map(item => (
                    <tr key={item.id} className="hover:bg-white/3 transition-colors" data-testid={`expense-row-${item.id}`}>
                      ...
                      <td className="px-6 py-4 text-right text-red-400 font-semibold">{formatCurrency(item.value)}</td>
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <button data-testid={`edit-expense-${item.id}`} onClick={() => openEdit(item)} className="p-1.5 rounded-lg text-slate-400 hover:text-blue-400 hover:bg-blue-400/10 transition-colors"><Pencil size={15} /></button>
                          <button data-testid={`delete-expense-${item.id}`} onClick={() => handleDelete(item)} className="p-1.5 rounded-lg text-slate-400 hover:text-red-400 hover:bg-red-400/10 transition-colors"><Trash2 size={15} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {modal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-card w-full max-w-md p-6 relative" data-testid="expense-modal">
            ...
            <h2 className="text-white font-bold text-xl mb-6" style={{ fontFamily: 'Manrope' }}>{modal === 'create' ? 'Nova Despesa' : 'Editar Despesa'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-slate-300 text-sm font-medium mb-1.5">Descrição *</label>
                <input data-testid="expense-form-name" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required className={inputClass} placeholder="Ex: Aluguel do mês" />
              </div>
              <div>
                <label className="block text-slate-300 text-sm font-medium mb-1.5">Valor (R$) *</label>
                <input data-testid="expense-form-value" type="number" step="0.01" min="0" value={form.value} onChange={e => setForm(p => ({ ...p, value: e.target.value }))} required className={inputClass} placeholder="0,00" />
              </div>
              <div>
                <label className="block text-slate-300 text-sm font-medium mb-1.5">Data *</label>
                <input data-testid="expense-form-date" type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} required className={inputClass} />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button data-testid="submit-expense" type="submit" disabled={submitting} className="flex-1 btn-pill py-2.5 text-sm disabled:opacity-50">
